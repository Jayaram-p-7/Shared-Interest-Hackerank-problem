#include<bits/stdc++.h>
using namespace std;
void product(vector<int> from,vector<int> to,vector<int> w) {
    map<int,set<int>> m;
    ...
    for(auto it=m.begin();it!=m.end();it++){
        ...
        int siz=v.size();
        for(int i=0;i<siz-1;i++) {p[make_pair(v[i],v[i+1])]++;
    ...
    for(auto itr=p.begin();itr!=p.end();itr++)
    {
        ...
        if(itr->second==max)
            if(max_product<pr.first*pr.second) max_product=pr.first*pr.second;
